...
const bookHero = defineType({
  ...
  fields: [
    ...heading("הספר"),
    defineField({ name: "subtitle", title: "משפט הפתיחה", type: "text", rows: 3 }),
    defineField({ name: "buyLabel", title: "כפתור ראשי", type: "string" }),
    defineField({
      name: "buyHref",
      title: "קישור הרכישה",
      type: "url",
      description: "הקישור לעמוד הרכישה. אפשר גם #buy כדי לגלול לאזור הרכישה בעמוד.",
      validation: (rule) => rule.uri({ allowRelative: true, scheme: ["http", "https"] }),
    }),
    defineField({ name: "readLabel", title: "כפתור משני", type: "string" }),
    defineField({
      name: "readHref",
